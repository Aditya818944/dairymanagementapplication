...
public class FarmerLedgerDialog extends JDialog 
{
private JTextField idField;
private JDateChooser fromDate, toDate;

public FarmerLedgerDialog(Window parent) {
...
private void generatePdf(java.util.List<MilkCollectionInterface> ds)
{
SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
int farmerId=ds.get(0).getFarmerId();
Date from=ds.get(0).getCollectionDate();
Date to=ds.get(ds.size()-1).getCollectionDate();
/**
*
*
Pdf creation logic starts herer
*
*/
...
totalAmount=milkCollectionManager.getAmountByDateAndFarmerId(farmerId,from,to);

nameToPrint=FarmerManager.getFarmerManager().getFarmerById(farmerId).getName();

pdfFileName=nameToPrint+" Ledger From ("+sdf.format(from)+" ) to ("+sdf.format(to)+").pdf";


if(FarmerManager.getFarmerManager().getFarmerById(farmerId).getType().equalsIgnoreCase("SPECIAL"))
{
ratePerFat=SpecialFarmerRateManager.getSpecialFarmerRateManager().getRate(farmerId,from);
ratePerFatString=ratePerFat.toString();
}else
{
ratePerFat=FarmerRateManager.getFarmerRateManager().getRate(from);
ratePerFatString=ratePerFat.toString();
}
...
for(MilkCollectionInterface m : ds)
{

String dateString=sdf.format(m.getCollectionDate());
String shiftString="";
if(m.getShift().equalsIgnoreCase("AM")) shiftString="Morning";
else shiftString="Evening";
String fatValueString=m.getFatValue().setScale(1,RoundingMode.HALF_UP).toString();
...
{
MilkCollectionManagerInterface milkCollectionManager=MilkCollectionManager.getMilkCollectionManager();
BigDecimal amount=milkCollectionManager.getAmountByDateAndFarmerId(Integer.parseInt(id),from,to);

String message="Farmer Id           :  "+id+"\n\n"+"From Date          :  "+sdf.format(from)+"\n\n"+"To Date              :  "+sdf.format(to)+"\n\n"+"Total amount    :   "+amount.toString()+"\n\n";
...
try
{
java.util.List<MilkCollectionInterface> list=new ArrayList<>();
int farmerId=Integer.parseInt(id);
from=sdf.parse(sdf.format(from));
to=sdf.parse(sdf.format(to));
MilkCollectionManagerInterface milkCollectionManager=MilkCollectionManager.getMilkCollectionManager();
Set<MilkCollectionInterface> st=milkCollectionManager.getAllByFarmerId(farmerId);
if(st.size()==0) {
JOptionPane.showMessageDialog(this,"No record exist for given farmer id" ,"Errors",JOptionPane.ERROR_MESSAGE);
return ;
}
for(MilkCollectionInterface m : st){
Date date=m.getCollectionDate();
