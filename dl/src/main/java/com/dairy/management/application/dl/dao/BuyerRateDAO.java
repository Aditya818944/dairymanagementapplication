...
import java.text.*;
import java.math.*;
public class BuyerRateDAO implements BuyerRateDAOInterface
{
...
PreparedStatement preparedStatement=connection.prepareStatement("select * from rate_chart_buyer where start_date=? and end_date=?");
...
preparedStatement=connection.prepareStatement("select start_date , end_date  from rate_chart_buyer");
...
preparedStatement=connection.prepareStatement("insert into rate_chart_buyer(rate_per_fat,start_date,end_date) values(?,?,?)",Statement.RETURN_GENERATED_KEYS);
...
PreparedStatement preparedStatement=connection.prepareStatement("select  rate_per_fat from rate_chart_buyer where start_date<=? and end_date>=?");
...
{
List<BuyerRateDTOInterface> list=new ArrayList<>();
try
{
Class.forName("com.mysql.cj.jdbc.Driver");
Connection connection=DriverManager.getConnection("jdbc:mysql://localhost:3306/tstdb","tstdbuser","tstdbuser");
PreparedStatement preparedStatement=connection.prepareStatement("select  rate_id , rate_per_fat,start_date,end_date from rate_chart_buyer;");
ResultSet resultSet=preparedStatement.executeQuery();
if(resultSet.next()==false)
{
resultSet.close();
...
BuyerRateDTOInterface buyerRate=new BuyerRateDTO();
...
Class.forName("com.mysql.cj.jdbc.Driver");
Connection connection=DriverManager.getConnection("jdbc:mysql://localhost:3306/tstdb","tstdbuser","tstdbuser");
PreparedStatement preparedStatement=connection.prepareStatement("select  rate_id , rate_per_fat,start_date,end_date from rate_chart_buyer where rate_id=?");
