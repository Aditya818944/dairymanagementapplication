...
import java.text.*;
import java.math.*;
public class FarmerRateDAO implements FarmerRateDAOInterface
{
...
PreparedStatement preparedStatement=connection.prepareStatement("select * from rate_chart_farmer where start_date=? and end_date=?");
...
connection.close();
throw new DAOException("Rate is already declared for given date : "+sdf.format(startDate)+" or "+sdf.format(endDate));
}




preparedStatement=connection.prepareStatement("select start_date , end_date  from rate_chart_farmer");
...
preparedStatement=connection.prepareStatement("insert into rate_chart_farmer(rate_per_fat,start_date,end_date) values(?,?,?)",Statement.RETURN_GENERATED_KEYS);
preparedStatement.setBigDecimal(1,ratePerFat);
...
public BigDecimal getRate(java.util.Date date) throws DAOException
{
try
{
...
PreparedStatement preparedStatement=connection.prepareStatement("select  rate_per_fat from rate_chart_farmer where start_date<=? and end_date>=?");
...
preparedStatement.close();
connection.close();
throw new DAOException("Rate is not declared for given date : "+sdf.format(date)+" , set rate for given date ");
}
...
{
List<FarmerRateDTOInterface> list=new ArrayList<>();
try
{
Class.forName("com.mysql.cj.jdbc.Driver");
Connection connection=DriverManager.getConnection("jdbc:mysql://localhost:3306/tstdb","tstdbuser","tstdbuser");
PreparedStatement preparedStatement=connection.prepareStatement("select  rate_id , rate_per_fat,start_date,end_date from rate_chart_farmer;");
ResultSet resultSet=preparedStatement.executeQuery();
if(resultSet.next()==false)
{
resultSet.close();
...
FarmerRateDTOInterface farmerRate=new FarmerRateDTO();
...
Class.forName("com.mysql.cj.jdbc.Driver");
Connection connection=DriverManager.getConnection("jdbc:mysql://localhost:3306/tstdb","tstdbuser","tstdbuser");
PreparedStatement preparedStatement=connection.prepareStatement("select  rate_id , rate_per_fat,start_date,end_date from rate_chart_farmer where rate_id=?");
...
public void update(FarmerRateDTOInterface rateChartBuyerDTO) throws DAOException
{
